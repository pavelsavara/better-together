;; Micro — a Better Together gardener hand-written in raw component-model WAT.
;;
;; Personality: the smallest possible *conditional cooperator*. Micro reads only
...
;;              This is honest — Micro really does decide from the table's talk.
...
;;
;; Build:  wasm-tools parse src/micro.wat -o micro.wasm
...
(component $micro
  ...
  (type $io-streams-iface (instance
    ...
    (alias outer $micro $error (type))                                     ;; 1
  ...
  (type $stdout-iface (instance
    (alias outer $micro $output-stream (type))                             ;; 0
  ...
  ;; =====================================================================
  ;; The exported resource type (representation = i32). Micro is stateless,
