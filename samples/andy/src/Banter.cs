...
internal static class Banter
{
    internal static void Talk(int round, int plant, Sig sig, bool wary)
    {
        ...
        Console.WriteLine($"[andy] round {round}: {mood}, I mean to tuck in {plant} seeds ({sig}).");
    }

    internal static void Plant(int round, int plant)
    {
        Console.WriteLine($"[andy] round {round}: padding out to plant {plant}. A promise is a promise.");
    }
